  ...
  return (
    <div className="flex flex-col gap-4">
      {categoryTotals.map((entry) => {
        ...
        // Com limite definido, a barra mostra o progresso desse limite;
        // sem limite, mostra a fatia do total gasto no mês (para
        // comparares categorias entre si).
        const displayPercentage = budgetRatio !== null ? budgetRatio * 100 : entry.percentage;
          ...
          <div key={entry.categoryId ?? "sem-categoria"}>
            <div className="mb-1 flex flex-wrap items-baseline justify-between gap-x-2 text-sm">
              ...
              <span className={isOverBudget ? "text-red-500" : "text-zinc-400"}>
                {formatCurrency(entry.total)} · {displayPercentage.toFixed(0)}%
